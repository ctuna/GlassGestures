...
public class ConnectionManager {
	...
	public ConnectionManager(MainActivity master, boolean isActive) {
		this.master = master;
		...
		if (isActive) {
			...
			if (pairedDevices.size() > 0) {
				...
				for (BluetoothDevice device : pairedDevices) {
					Log.i("debugging", "in for, we want device address: "
							+ connectionAddress);
					if (D) {
						Log.i("debugging", "Device is: " + device.getName());
						Log.i("debugging",
	...
	private class ConnectThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				// "the server is not available", Toast.LENGTH_SHORT);
				// toast.show();
				try {
					mmSocket.close();
		...
		public synchronized void connected(BluetoothSocket socket) {
			...
			Log.i("debugging", "name of socket device is: "
	...
	private class ConnectedThread extends Thread {
		...
		public void write(byte[] bytes) {
			if (isActive) {
				Log.i("debugging", "in connectedthread.write writing "
						+ new String(bytes));
				try {
	...
	public synchronized void restartConnection() {
		if (D)
			Log.i("debugging", "restarting connection ");
	...
	public String formatMessage(ControlledObject object, Variable variable,
			char function, String... currentValue) {
		...
		if (function != 'R') {
			...
			if (currentValue[0].equals("off")) {
				...
				if (!currentValue[0].equals("INC")
						&& !currentValue[0].equals("DEC")) {
					val = " ON";
					function = 'C';
				} else {
					function = 'C';
